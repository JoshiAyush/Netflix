...
import { ExtractDataFromClient } from "../browser-report.js";

export function enableTesting() {
    window.describe("User Agent Matching", function () {
        getExpectedResults().forEach(function (item) {
            ...
            window.it(message, function () {
                ...
                if (!(report["os"]["name"] == expected["os"]["name"]))
                    console.error(`Report: ${report} doesn't match with Expected: ${expected}`);
